# mymemcached ”ÎSpring’˚∫œ Simple Spring Memcached
Memcached  
Introduction
Dependencies
Configuration
...
Check if value exists in the cache and can be cast to number. If it does, return it and exit cleanly.
If not found in the cache, then ask the underlying method for the data.
Before returning the data, write it out to the cache (using incr command) for future access.
...
Spring 3.1 Cache Integration
Spring 3.1 introduced new cache abstraction. Out of the box spring provides integration with two storages ConcurrentMap and ehcache. Simple Spring Memcached can be used as another storage. Using SSM users of Spring 3.1 Cache can store data in memcached.
